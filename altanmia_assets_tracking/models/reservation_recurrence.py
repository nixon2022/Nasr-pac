...
class RecurrenceRule(models.Model):
    _name = 'tanmia.assets.recurrence'
    _description = 'Reservation Recurrence Rule'

    name = fields.Char(compute='_compute_name', store=True)
    base_event_id = fields.Many2one(
        'tanmia.assets.reservation', ondelete='set null', copy=False)  # store=False ?
    calendar_event_ids = fields.One2many('tanmia.assets.reservation', 'recurrence_id')
    ...
    def _compute_dtstart(self):
        groups = self.env['tanmia.assets.reservation'].read_group([('recurrence_id', 'in', self.ids)], ['start:min'], ['recurrence_id'])
        start_mapping = {
    ...
    def _apply_recurrence(self, specific_values_creation=None, no_send_edit=False, generic_values_creation=None):
        ...
        """
        event_vals = []
        keep = self.env['tanmia.assets.reservation']
        ...
        self.env['tanmia.assets.reservation'].with_context(no_mail_to_attendees=True, mail_create_nolog=True).create(event_vals)
    ...
    def _get_first_event(self, include_outliers=False):
        if not self.calendar_event_ids:
            return self.env['tanmia.assets.reservation']
    ...
    def _get_outliers(self):
        synced_events = self.env['tanmia.assets.reservation']
        for recurrence in self:
    ...
    def _get_events_from(self, dtstart):
        return self.env['tanmia.assets.reservation'].search([
