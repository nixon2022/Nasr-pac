...
from ..controllers.main import HyperPayController
...
class PaypalForm(PaypalCommon):
    ...
    def _get_expected_values(self):
        return_url = self._build_url(HyperPayController._return_url)
        values = {
            ...
            'last_name': 'Buyer',
            'lc': 'en_US',
            'notify_url': self._build_url(HyperPayController._notify_url),
            'return': return_url,
            'rm': '2',
            'zip': '1000',
        }
    ...
    def test_parsing_pdt_validation_response_returns_notification_data(self):
        ...
        notification_data = HyperPayController._parse_return_validation_response(response_content)
